...
class Coordinate : public Estimator<DedupedPack> {
  public:
  explicit Coordinate(ParameterPack<DedupedPack> parameters, int num_evals, float norm = 1e-3f)
  ...
  void perturb() override {
    this->apply(
      current_,
      state_.is_pos ? state_.norm : -state_.norm
    );
  }
  ...
  void observe(float loss) override {
    ...
    if (state_.is_pos) {
      ...
      state_.grad[current_] =
        (state_.pos_mean - state_.neg_mean) / (2.0f * state_.norm);
