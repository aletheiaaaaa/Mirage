...
namespace agon::simd {
  ...
  inline Vec<CURRENT_ARCH, T> add(const Vec<CURRENT_ARCH, T>& a, const Vec<CURRENT_ARCH, T>& b) {
    if constexpr (std::is_same_v<T, int8_t>) {
      ...
      return Vec<CURRENT_ARCH, T>(_mm512_add_epi32(a.data, b.data));
  ...
  inline Vec<CURRENT_ARCH, T> add(const Vec<CURRENT_ARCH, T>& a, const Vec<CURRENT_ARCH, T>& b) {
    if constexpr (std::is_same_v<T, int8_t>) {
      ...
      return Vec<CURRENT_ARCH, T>(_mm256_add_epi32(a.data, b.data));
    } else if constexpr (std::is_same_v<T, float>) {
  ...
  inline Vec<CURRENT_ARCH, T> add(const Vec<CURRENT_ARCH, T>& a, const Vec<CURRENT_ARCH, T>& b) {
    if constexpr (std::is_same_v<T, int8_t>) {
      ...
      return Vec<CURRENT_ARCH, T>(_mm_add_epi32(a.data, b.data));
    } else if constexpr (std::is_same_v<T, float>) {
