    ...
    inline T cast(F v) {
        ...
        if constexpr (std::is_same_v<F, VecI8<arch>>) {
            ...
            if constexpr (std::is_same_v<T, VecI64<arch>>) {
                ...
                static_assert(std::is_same_v<T, void>, "Unsupported cast from VecI32<AVX512>");
            }
        }
            ...
            if constexpr (std::is_same_v<T, VecI32<arch>>) {
                ...
                static_assert(std::is_same_v<T, void>, "Unsupported cast from VecI64<AVX512>");
            }
        }
            ...
            if constexpr (std::is_same_v<T, VecF64<arch>>) {
                ...
                return T(_mm512_cvtps_epi32(v.data));

            } else if constexpr (std::is_same_v<T, VecI64<arch>>) {
                __m256 chunk = (N == 0) ? _mm512_castps512_ps256(v.data) : _mm512_extractf32x8_ps(v.data, 1);
                return T(_mm512_cvtps_epi64(chunk));
            ...
            if constexpr (std::is_same_v<T, VecF32<arch>>) {
                ...
                return T(_mm512_cvtpd_epi64(v.data));
    ...
    inline T cast(F v) {
        ...
        if constexpr (std::is_same_v<F, VecI8<arch>>) {
            ...
            if constexpr (std::is_same_v<T, VecI64<arch>>) {
                ...
                static_assert(std::is_same_v<T, void>, "Unsupported cast from VecI32<AVX2>");
            }
        }
            ...
            if constexpr (std::is_same_v<T, VecI32<arch>>) {
                ...
                static_assert(std::is_same_v<T, void>, "Unsupported cast from VecI64<AVX2>");
            }
        }

        else if constexpr (std::is_same_v<F, VecI16<arch>> && std::is_same_v<T, VecI8<arch>>) {
            __m256i shuf = _mm256_set_epi8(
            ...
            if constexpr (std::is_same_v<T, VecF64<arch>>) {
                ...
                return T(_mm256_cvtps_epi32(v.data));
    ...
    inline T cast(F v) {
        ...
        if constexpr (std::is_same_v<F, VecI8<arch>>) {
            ...
            if constexpr (std::is_same_v<T, VecI64<arch>>) {
                ...
                static_assert(std::is_same_v<T, void>, "Unsupported cast from VecI32<SSE4_1>");
            }
        }
            ...
            if constexpr (std::is_same_v<T, VecI32<arch>>) {
                ...
                static_assert(std::is_same_v<T, void>, "Unsupported cast from VecI64<SSE4_1>");
            }
        }
            ...
            if constexpr (std::is_same_v<T, VecF64<arch>>) {
                ...
                return T(_mm_cvtps_epi32(v.data));
            } else if constexpr (std::is_same_v<T, VecI16<arch>>) {
            ...
            if constexpr (std::is_same_v<T, VecF32<arch>>) {
                ...
                return T(_mm_cvtpd_epi32(v.data));
            } else if constexpr (std::is_same_v<T, VecI16<arch>>) {
    ...
    inline T cast(F v) {
        ...
        if constexpr (std::is_same_v<F, VecI8<arch>>) {
            ...
            if constexpr (std::is_same_v<T, VecI64<arch>>) {
                ...
                static_assert(std::is_same_v<T, void>, "Unsupported cast from VecI32<GENERIC>");
            }
        }
        else if constexpr (std::is_same_v<F, VecI64<arch>>) {
            if constexpr (std::is_same_v<T, VecI32<arch>>) {
                ...
                static_assert(std::is_same_v<T, void>, "Unsupported cast from VecI64<GENERIC>");
            }
        }
            ...
            if constexpr (std::is_same_v<T, VecF64<arch>>) {
                ...
                return T(std::lround(v.data));
            } else if constexpr (std::is_same_v<T, VecI64<arch>>) {
                return T(std::llround(v.data));
            } else if constexpr (std::is_same_v<T, VecI16<arch>>) {
